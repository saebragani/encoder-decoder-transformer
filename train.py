...
def greedy_decoder(model, target_tokenizer, encoder_input, encoder_mask, device, max_translation_len):
    ...
    while last_word_token_id != eos and decoder_input.shape[1] < max_translation_len:
        ...
        causal_mask = (
            torch.tril(torch.ones(decoder_input.shape[1], decoder_input.shape[1])).type_as(encoder_mask).to(device)
        )
        decoder_output = model.decode(decoder_input, encoder_output, encoder_mask, causal_mask)
        projection = model.project(
            decoder_output
        )  # (B, target_seq_len, target_vocab_size) **target_seq_len growing here**
        ...
        decoder_input = torch.cat(
            [decoder_input, last_word_token_id.view(1, 1).type_as(decoder_input).to(device)], dim=1
        )
...
def run_validation(
    model, data_loader, target_tokenizer, target_seq_len, device, print_msg, writer, global_step, max_num_val_steps=None
):
    model.eval()
    ...
    if writer:
        ...
        cer = metric(translation_sentence_ls, target_sentence_ls)
        writer.add_scalar("validation CER", cer, global_step)
        writer.flush()

        # Log the word error rate
        metric = torchmetrics.WordErrorRate()
        wer = metric(translation_sentence_ls, target_sentence_ls)
        writer.add_scalar("validation WER", wer, global_step)
        writer.flush()

        # Log the BLEU metric
        metric = torchmetrics.BLEUScore()
        bleu = metric(translation_sentence_ls, target_sentence_ls)
...
def train_model(
    ...
    model, train_data_loader, val_data_loader, trained_tokens_dict = prepare_model_data(
